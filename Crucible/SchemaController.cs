...
namespace schemaforge.Crucible
{
    public abstract class SchemaController
    {
        ...
        {
            string message = " ";
            // This option is included in case a sub-JObject of another configuration is being validated; this allows the ErrorList to indicate the exact configuration that has the issue. Name is usually the token name of the sub-configuration.
            ...
            The decision to invalidate the config due to unrecognized tokens stems from the possibility that an end user might misspell an optional token when forming their configuration file or request.
            
            If the user includes an optional token with a typo in the token name, it will not be flagged as a missing required token, but it will also not have the effect the user intended from including the optional token.

            Such a problem would be very frustrating and possibly difficult to debug; therefore, we invalidate the config file if there are any tokens that are not accounted for in Required and Optional put together.
        ...
        /// <summary>
        /// Returns the current schema controller as a stringified Json object, filling in values for required and optional config tokens if UserConfig has already been populated.
        /// </summary>
        ...
        /// </summary>
        /// <returns>A JObject file with all tokens in RequiredConfigTokens and OptionalConfigTokens. If the HelpStrings are well-written, the return value will serve as an excellent example for an end user to fill in.</returns>
        ...
        */
        ...
        /// <summary>
        /// Produces ValidationFunction members for ConfigTokens, which are executed on the corresponding values found in the UserConfig property. If the value of the token in the user's config can be cast as T1, then constraintsIfT1 are executed.
